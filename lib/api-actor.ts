...
import { safeEqual } from "@/lib/crypto-equal";
import {
  getApiSessionUserId,
...
function sb() {
  ...
  return createWarriorBrowserClient();
}
...
export function isServerAdminSecret(headerValue: string | null | undefined): boolean {
  ...
  return safeEqual(headerValue.trim(), secret);
}
...
> {
  if (isServerAdminSecret(opts.adminSecret)) {
    return { ok: true, role: "secret-admin", canDelete: true, actorId: null };
  }
  ...
  const role = await fetchActorRole(actor.actorId);
  if (role === "admin") {
    return { ok: true, role, canDelete: true, actorId: actor.actorId };
  }
  if (!opts.write && role === "coach") {
    return { ok: true, role, canDelete: false, actorId: actor.actorId };
  }
